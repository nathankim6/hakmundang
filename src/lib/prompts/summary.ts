...
1. 지문의 전체 내용을 요약하는 문장을 만드세요. 이 문장은 주어진 지문의 핵심 내용을 정확하게 반영해야 하며, 두 개의 빈칸 (A)와 (B)가 포함되어야 합니다.
...
[지문의 내용에 맞게 작성된 요약문. (A)와 (B)는 빈칸으로 둡니다.]
...
위의 지침에 따라 주어진 지문을 바탕으로 문제를 생성하고 정답을 제공해주세요. 매번 문제를 생성할 때마다 정답의 위치가 무작위로 변경되도록 해주세요. 요약문은 반드시 주어진 지문의 내용에 맞게 새롭게 작성되어야 합니다.`;