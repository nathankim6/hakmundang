...
| | | 훈련은 **① 구문 1 · ② 구문 2 · ③ 두 구문 동시 적용** 순서로 새로 쓴 문장을 쓴다 |
...
- **먼저 보기**는 접속사가 있는 문장을 골라 완전히 표시된 상태로 보여 준다(훈련 3문장과 중복 금지).
  주어·종속절 주어 초록 실선 밑줄 · 본동사 네이비 · 접속사 골드 네모 · 수식어 회색 밑줄.
  라벨은 **S · V · M · S′ · V′ 기호만** 쓰고 한글을 적지 않는다. △ 는 해당 단어 **글자 위에 겹쳐** 얹는다
  (`.tk{padding-top:0}` + `.tk em{position:absolute; top:1px}`). △ 는 글리프가 아니라 인라인 SVG 라
  `stroke-width` 로 선 두께를 직접 조절한다.
  현재 값: 렌더 크기 `18×15px`(CSS) · `stroke-width 1.6`(viewBox 14×12 기준 → 실제 약 2.06px).
  크기를 키울 때는 stroke-width 도 같은 비율로 굵어지므로 두께를 유지하려면 함께 낮춘다.
