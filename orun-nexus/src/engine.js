...
uniform vec2 u_res;uniform float u_seed;uniform vec3 u_ac0,u_ac1,u_ac2,u_ac3,u_ac4;uniform vec2 u_np0,u_np1,u_np2,u_np3,u_np4;`+GLSL_NOISE+`
...
void main(){
  ...
  /* 별 3층 */
  col+=starLayer(px,9.0,0.16,0.55,0.42,u_seed);
  col+=starLayer(px,26.0,0.30,0.85,0.72,u_seed+3.0);
  col+=starLayer(px,70.0,0.30,1.30,1.00,u_seed+9.0);
  /* 회절 스파이크가 선 밝은 별 */
  for(int i=0;i<9;i++){
    float fi=float(i);
    vec2 sp=vec2(hash21(vec2(fi,1.7)+u_seed),hash21(vec2(fi,4.3)+u_seed))*u_res;
    vec2 d=px-sp; float dd=dot(d,d);
    float sz=1.3+1.6*hash21(vec2(fi,8.1));
    ...
    float spike=(exp(-abs(d.x)/(15.0*sz))*exp(-abs(d.y)*1.6)+exp(-abs(d.y)/(15.0*sz))*exp(-abs(d.x)*1.6))*0.26;
  ...
  for(int i=0;i<7;i++){
    ...
    float rot=hash21(vec2(fi,3.3))*3.14159, ax=8.0+14.0*hash21(vec2(fi,4.4)), ay=ax*(0.28+0.5*hash21(vec2(fi,5.5)));
...
function initGL(){
  if(!glcv) return;
  let gl=null;
  try{ gl=glcv.getContext('webgl',{alpha:true,premultipliedAlpha:true,antialias:false,depth:false,stencil:false,preserveDrawingBuffer:false}); }catch(e){ gl=null; }
  if(!gl) return;
  ...
  gl.enable(gl.BLEND);
  GL={gl,prog:P0.prog,uAlpha:P0.u.u_alpha,uTex:P0.u.u_tex,buf,verts:new Float32Array(24),sky:PSKY,fin:PFIN,neb:null,nebKey:''};
...
function glBake(w,h,P,setup,mip){
  ...
  if(mip){ gl.bindTexture(gl.TEXTURE_2D,tex); gl.generateMipmap(gl.TEXTURE_2D); gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR_MIPMAP_LINEAR); }
  return tex;
}
/* 하늘 — 화면 크기마다 한 번 */
...
function glSky(){
  ...
  const w=glcv.width, h=glcv.height;
  GL.neb=glBake(w,h,GL.sky,(gl,u)=>{
    gl.uniform2f(u.u_res,w,h); gl.uniform1f(u.u_seed,3.7);
...
function governor(now){
  ...
  ftLast=now;
  if(ftAvg>27 && QUALITY>0){ QUALITY--; ftAvg=16; STAGE_EL.classList.toggle('lowq',QUALITY===0); }
...
const GAL_R=596*1.4, TEX=1024, TEXR=470;
const GAL_TYPES={
...
function buildGalaxyTexture(gal){
  ...
  const C=TEX/2, b=Math.tan(P.pitch), r0=TEXR*0.06;
  ...
  for(let i=0;i<14000;i++){
    ...
    const sz=rnd()<.85?1+rnd()*1.2:2+rnd()*1.6, a=.25+rnd()*.6;
  ...
  for(let i=0;i<70;i++){
    ...
    g.globalAlpha=.9; g.fillStyle='rgba(255,255,255,1)'; g.fillRect(x-.8,y-.8,1.6,1.6);
  }
  const nh=Math.round(170*P.hii);
  for(let i=0;i<nh;i++){
    const r=P.ring?rRing():TEXR*.15+rnd()*TEXR*.7, m=i%P.arms;
    const [x,y]=place(r,m,P.scatter*.6);
    const sz=8+rnd()*14;
    g.globalAlpha=.22+rnd()*.28; g.drawImage(sprite(hiiC,32),x-sz/2,y-sz/2,sz,sz);
    if(rnd()<.6){ const s2=5+rnd()*9; g.globalAlpha=.3; g.drawImage(sprite(obC,32),x+gauss()*6-s2/2,y+gauss()*6-s2/2,s2,s2); }
  }
  /* 6. 코어 */
  g.globalAlpha=1;
...
(async function start(){
  ...
  try{
    const gc=document.createElement('canvas'); gc.width=gc.height=192;
    const gx=gc.getContext('2d'), im=gx.createImageData(192,192), d=im.data;
