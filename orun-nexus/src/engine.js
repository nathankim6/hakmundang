...
/* 우주 배치. pos 는 우주 좌표(x 가로 · y 높이(-가 위) · z 깊이)이고 세 은하의
   ...
   파고들면 그 은하만 수평으로 돌아온다(oAmt). accent 는 은하의 고유 전류색,
...
const GALAXIES=[
  { id:'grammar', name:'GRAMMAR', kr:'문법 은하', tagline:'중등 문법 커리큘럼',
    ...
    pos:{x:-1240,y:-30,z:240}, orient:{rx:0.15,rz:-0.10},
  ...
  { id:'reading', name:'READING', kr:'독해 은하', tagline:'READING GRAPHY · 옳은 독해',
    ...
    pos:{x:1210,y:-130,z:260}, orient:{rx:-0.22,rz:0.18},
  ...
  { id:'vocab', name:'VOCAB', kr:'보카 은하', tagline:'옳은보카 VOL.0 – ULTIMATE',
    ...
    pos:{x:30,y:160,z:-500},   orient:{rx:0.30,rz:-0.26},
...
const PITCH_MIN=0.30, PITCH_MAX=1.36;
...
new ResizeObserver(()=>{resize();buildBackdrop();}).observe(document.getElementById('stage'));
resize();
...
function frameAt(z){
  ...
  if(expandAll){
    add(0,0,40,40);                            /* the ring itself sets the frame */
  } else {
    const tip=project(0,-244,0), foot=project(0,0,0);
    add(tip.sx,tip.sy,62,46); add(foot.sx,foot.sy,86,40);
  }
  live.forEach(n=>{
    const p=project(n.tx,lift(n),n.ty);
    /* 파고든 화면은 노드가 몇 개뿐이라 예전 상수 여백으로도 잘리지 않고,
       그 편이 등대를 가운데 두는 구도가 나온다. 실측은 이름표가 사방으로
       뻗어 잘리던 은하에만 쓴다. */
    if(!expandAll){
      const pad = n.kind==='item'?176: n.kind==='chap'?152: 104;
      add(p.sx,p.sy,pad*p.k,42*p.k); return;
    }
    if(n.kind==='book'){ add(p.sx,p.sy,104*p.k,54*p.k); return; }
    const R=nodeStyle(n).rad*p.k*z;
    let t=n.label||''; const cap=n.kind==='item'?22:20;
    if(t.length>cap) t=t.slice(0,cap-1)+'…';
    const reach=(R+10+mw(t,labelFont(n,p.k,z))+8)*inv;
    const dl=Math.hypot(p.sx,p.sy)||1;
    add(p.sx+p.sx/dl*reach, p.sy+p.sy/dl*reach, 4, 9*inv);
    add(p.sx,p.sy,(R+4)*inv,(R+4)*inv);
  });
  const w=Math.max(1,x1-x0), h=Math.max(1,y1-y0);
  return {z:Math.max(.4,Math.min(2.1,Math.min(W/w,H/h)*0.98)), cx:(x0+x1)/2, cy:(y0+y1)/2};
}
function fit(){
  if(!live.length) return;
  framePend=framePend||Math.abs((expandAll?1:0)-galaxyAmt)>0.006;
  /* 배율 → 이름표 크기 → 차지하는 자리 → 배율. 서로 물려 있어 한 번에
     안 떨어진다. 몇 번 굴려 수렴시키고, 마지막은 작은 쪽을 택해 잘림보다
     여백 쪽으로 틀리게 한다. */
  let f=frameAt(cam.zoom), z=f.z;
...
/* 우주 프레임: 세 은하의 원반과 이름판이 모두 들어오게 잡는다.
   towerUpright 로 기울기를 끄고 재므로 근사치지만, 원반 반지름에
   여유를 실어 잘림보다 여백 쪽으로 틀린다. */
function universeFit(){
  ...
  GALAXIES.forEach(g=>{
    setGOFF(g);
    towerUpright=true;
    const R=RADII[3]*1.30;
    [[0,-360,0],[R,0,0],[-R,0,0],[0,0,R],[0,0,-R],[0,80,0]].forEach(v=>{
      const q2=project(v[0],v[1],v[2]);
      x0=Math.min(x0,q2.sx); x1=Math.max(x1,q2.sx);
      y0=Math.min(y0,q2.sy); y1=Math.max(y1,q2.sy);
    });
    towerUpright=false;
  });
  GOFF.x=kx; GOFF.y=ky; GOFF.z=kz;
  const w=Math.max(1,x1-x0+260), h=Math.max(1,y1-y0+190);
  const z=Math.max(.13,Math.min(1.1,Math.min(W/w,H/h)));
  cam.tzoom=z;
  cam.tpx=-(x0+x1)/2*z;
  cam.tpy=-(y0+y1)/2*z;
}
...
function draw(ts){
  ...
  if(autoFrame){
    const gliding = Math.abs(WCT.x-WC.x)+Math.abs(WCT.y-WC.y)+Math.abs(WCT.z-WC.z)>2
                 || Math.abs((universeMode?1:0)-uAmt)>0.01;
    const opening = Math.abs((expandAll?1:0)-galaxyAmt)>0.006;
    if(gliding||opening){ (universeMode?universeFit:fit)(); framePend=true; }
    else if(framePend){ framePend=false; (universeMode?universeFit:fit)(); }
  }
  ...
  gOrder.forEach(o=>{
    ...
    withGalaxy(g,()=>{
      ...
      labelQ.length=0;
      drawNodes();
      g._labels=labelQ.slice(); labelQ.length=0;
      GDIM=1;
    });
  });
  bindGalaxy(ACTIVE);
...
function drawNameplates(){
  const list=GALAXIES.map(g=>({g:g,q:npAnchor(g)})).sort((a,b)=>b.q[3]-a.q[3]);
  list.forEach(o=>{
...
cv.addEventListener('pointermove',e=>{
  ...
  if(drag){
    ...
    cam.idle=0;
    if(drag.pan){ cam.tpx=drag.px+dx; cam.tpy=drag.py+dy; cam.px=cam.tpx; cam.py=cam.tpy; }
    else{
...
cv.addEventListener('wheel',e=>{
  ...
  cam.tzoom=Math.max(.2,Math.min(2.6,cam.tzoom*Math.exp(-e.deltaY*0.0012)));
},{passive:false});
...
function sync(doFit){
  layout(); modePitch();
  if(doFit!==false){ autoFrame=true; framePend=true; setTimeout(fit,30); }
  renderRails();
  renderCrumb();
}
...
function keyNav(){
  ...
  if(HELD.has('+')||HELD.has('=')) cam.tzoom=Math.min(2.6,cam.tzoom*1.022);
  if(HELD.has('-')||HELD.has('_')) cam.tzoom=Math.max(.2, cam.tzoom/1.022);
...
document.getElementById('btn-fit').onclick=()=>{
  pitchUser=false; modePitch(); autoFrame=true; framePend=true;
  (universeMode?universeFit:fit)();
};
(function(){
...
document.getElementById('btn-solo').onclick=function(){
  ...
  setTimeout(()=>{resize();autoFrame=true;framePend=true;fit();},380);
};
...
(async function start(){
  ...
  buildGalPick();
  syncGalaxyChrome();
  universeFit(); renderRails(); renderCrumb();
  requestAnimationFrame(draw);
  boot();
})();
