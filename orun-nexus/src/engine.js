...
function initGL(){
  ...
  if(!gl) return;
  const VS='attribute vec4 a_pos;attribute vec2 a_uv;varying vec2 v_uv;void main(){v_uv=a_uv;gl_Position=a_pos;}';
  const FS='precision mediump float;varying vec2 v_uv;uniform sampler2D u_tex;uniform float u_alpha;void main(){gl_FragColor=texture2D(u_tex,v_uv)*u_alpha;}';
  ...
  gl.enable(gl.BLEND);
  GL={gl,prog,buf,uAlpha:gl.getUniformLocation(prog,'u_alpha'),uTex:gl.getUniformLocation(prog,'u_tex'),verts:new Float32Array(24)};
  gl.uniform1i(GL.uTex,0);
...
function glGalaxyQuad(g,alpha){
  ...
  glDrawQuad(glTexture(tex,true),alpha,true);
}
...
function glFrame(){
  ...
  GALAXIES.forEach(g=>{
    ...
    withGalaxy(g,()=>{
      const A=(0.18+0.5*galaxyAmt+0.22*uAmt)*(back?0.2:1);
...
const GAL_R=596*1.25, TEX=1024, TEXR=470;
const GAL_TYPES={
...
function drawGalaxyTex(){
  ...
  if(!tex) return;
  const A=(0.18+0.5*galaxyAmt+0.22*uAmt)*GDIM;
  if(A<=.01) return;
