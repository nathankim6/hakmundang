...
옳은영어 커리큘럼 유니버스 — **한 하늘에 뜬 세 Galaxy**.
...
(GRAMMAR · READING · VOCAB)이 떠 있다. 이름판이나 Galaxy 원반을 클릭하면
그 Galaxy로 들어가고, ESC 또는 [우주로] 버튼으로 우주 화면에 돌아온다.
활성 Galaxy 안에서는 기존 넥서스와 동일하게 동작한다 — 교재 → 챕터/PART →
...
- `data/grammar.json` — 문법 Galaxy 데이터 (원본 넥서스에서 그대로 추출, 수정 금지 대상 아님).
