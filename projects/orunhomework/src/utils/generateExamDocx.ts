...
function buildScrambledLine(english: string, fullEnglish: string): TextRun[] {
  const isMultiPart = english.includes("|||");
  
  if (isMultiPart) {
    // Multi-part: combine all parts' words, scramble them, show non-selected parts as context
    const parts = english.split("|||").map(p => p.trim()).filter(Boolean);
    const regions = parts.map(p => {
      const idx = fullEnglish.indexOf(p);
      return idx !== -1 ? { start: idx, end: idx + p.length, text: p } : null;
    }).filter(Boolean).sort((a, b) => a!.start - b!.start) as { start: number; end: number; text: string }[];

    // Collect all words from all parts
    let allWords: string[] = [];
    for (const r of regions) {
      allWords.push(...splitWords(r.text));
    }
    allWords = removeTrailingPeriod(allWords);
    const scrambledWords = shuffle(allWords);

    // Build runs: context + [scrambled] + context
    const runs: TextRun[] = [];
    let cursor = 0;
    for (const r of regions) {
      if (cursor < r.start) {
        runs.push(new TextRun({
          text: fullEnglish.substring(cursor, r.start),
          size: 20,
          font: "맑은 고딕",
        }));
      }
    ...
    // Instead of inline per-region, show one big scrambled block
    const prefix = regions[0].start > 0 ? fullEnglish.substring(0, regions[0].start) : "";
    const suffix = fullEnglish.substring(regions[regions.length - 1].end);
    const middleParts: string[] = [];
    for (let i = 0; i < regions.length - 1; i++) {
      middleParts.push(fullEnglish.substring(regions[i].end, regions[i + 1].start));
    }

    const result: TextRun[] = [];
    if (prefix.trim()) {
      result.push(new TextRun({ text: prefix, size: 20, font: "맑은 고딕" }));
    }
    ...
    if (middleParts.length > 0) {
      for (const mid of middleParts) {
        if (mid.trim()) {
          result.push(new TextRun({ text: ` ${mid.trim()} `, size: 20, font: "맑은 고딕", color: "6B7280" }));
        }
      }
    }
    if (suffix.trim()) {
      result.push(new TextRun({ text: suffix, size: 20, font: "맑은 고딕" }));
    }
    return result;
  }
