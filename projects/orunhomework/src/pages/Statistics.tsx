...
import { BarChart3, Download, TrendingUp, TrendingDown, Users, Target, Clock, Search, Award, Trophy, Star } from "lucide-react";
...
import BulkMonthlyReportDialog from "@/components/statistics/BulkMonthlyReportDialog";
...
function StudentSearchCard({
  ...
  const schools = useMemo(() => {
    const set = new Set(allStudentStats.map(s => s.school));
    return Array.from(set).sort();
  }, [allStudentStats]);

  const grades = useMemo(() => {
    const filtered = selectedSchool === "all" ? allStudentStats : allStudentStats.filter(s => s.school === selectedSchool);
    const set = new Set(filtered.map(s => s.grade));
    return Array.from(set).sort();
  }, [allStudentStats, selectedSchool]);
  ...
  }, [allStudentStats, searchQuery, selectedSchool, selectedGrade]);
...
function ScholarshipSection({
  ...
  allStudentStats: (StudentStat & { lateCount?: number })[];
  effectiveStudentIds: string[] | null;
  studentIdsReady: boolean;
  ...
  const { data: rtData } = useQuery({
    queryKey: ["scholarship-rt", startOfMonthStr, todayStr, effectiveStudentIds],
    enabled: studentIdsReady && allStudentStats.length > 0,
    queryFn: async () => {
      const studentIds = allStudentStats.map(s => s.id);
      if (studentIds.length === 0) return {};

      // 이번 달 숙제 목록
      const { data: homeworkList } = await supabase
        .from("homework")
        .select("id, target_grade_id, target_student_id, target_type")
      ...
      // 학생별 배정 숙제 수 & 제출 수
      const result: Record<string, { assigned: number; submitted: number }> = {};
      ...
      const { data: studentsWithGrade } = await supabase
        ...
        .select("id, grade_id")
        .in("id", studentIds);
      (studentsWithGrade || []).forEach((s: any) => { studentGradeMap[s.id] = s.grade_id; });

      studentIds.forEach(sid => {
        const gradeId = studentGradeMap[sid];
        const assignedHw = homeworkList.filter(h => {
          if (h.target_type === "individual") return h.target_student_id === sid;
          if (h.target_type === "grade") return h.target_grade_id === gradeId;
          return false;
        });
        const submittedCount = (submissions || []).filter(s => 
          s.student_id === sid && (s.status === "submitted" || s.status === "reviewed")
        ).length;
        result[sid] = { assigned: assignedHw.length, submitted: submittedCount };
      });
  ...
  // 영작(writing) 정답률 (이번 달)
  const { data: writingData } = useQuery({
    queryKey: ["scholarship-writing", startOfMonthStr, todayStr, effectiveStudentIds],
    enabled: studentIdsReady && allStudentStats.length > 0,
    queryFn: async () => {
      const studentIds = allStudentStats.map(s => s.id);
      if (studentIds.length === 0) return {};
      ...
      (submissions || []).forEach((s: any) => {
        if (!result[s.student_id]) result[s.student_id] = { total: 0, correct: 0 };
        result[s.student_id].total++;
        if (s.is_correct) result[s.student_id].correct++;
      });
      return result;
    },
  });
  ...
  // 평균 제출시간 (이번 달, 학생별)
  const { data: submitTimeData } = useQuery({
    queryKey: ["scholarship-submit-time", startOfMonthStr, todayStr, effectiveStudentIds],
    enabled: studentIdsReady && allStudentStats.length > 0,
    queryFn: async () => {
      const studentIds = allStudentStats.map(s => s.id);
      if (studentIds.length === 0) return {};
      ...
      const { data: submissions } = await supabase
        ...
        .in("student_id", studentIds)
        .gte("submission_date", startOfMonthStr)
      ...
      (submissions || []).forEach((s: any) => {
        ...
        const minutes = kstHour * 60 + d.getMinutes();
        if (!result[s.student_id]) { result[s.student_id] = 0; counts[s.student_id] = 0; }
        result[s.student_id] += minutes;
        counts[s.student_id]++;
      });

      const avg: Record<string, number> = {};
      Object.keys(result).forEach(id => {
        avg[id] = Math.round(result[id] / counts[id]);
      });
      return avg;
    },
  });

  // 종합 점수 계산 및 반별 Top 2 선정
  const scholarshipByGrade = useMemo(() => {
    if (allStudentStats.length === 0) return [];

    const scored: ScholarshipStudent[] = allStudentStats.map(s => {
      const completionRate = s.rate;
      const lateCount = (s as any).lateCount || 0;
      const submissionCount = s.submissionCount || 0;
      const onTimeRate = submissionCount > 0 ? Math.round(((submissionCount - lateCount) / submissionCount) * 100) : 0;

      const rt = rtData?.[s.id];
      const rtSubmissionCount = rt?.submitted || 0;
      const rtTotalCount = rt?.assigned || 0;
      const rtRate = rtTotalCount > 0 ? (rtSubmissionCount / rtTotalCount) * 100 : 100; // 배정 없으면 만점

      const writing = writingData?.[s.id];
      const writingAccuracy = writing && writing.total > 0 ? Math.round((writing.correct / writing.total) * 100) : 0;
      const writingTotal = writing?.total || 0;

      // 제출시간 점수 (오전 일찍 제출 = 높은 점수, 최대 15점)
      const avgTime = submitTimeData?.[s.id]; // 분 단위 (0~1440)
      let timeScore = 0;
      if (avgTime !== undefined) {
        // 0시~8시(480분 이하) = 15점, 8~12시 = 12점, 12~18시 = 8점, 18~24시 = 4점
        if (avgTime <= 480) timeScore = 15;
        else if (avgTime <= 720) timeScore = 12;
        else if (avgTime <= 1080) timeScore = 8;
        else timeScore = 4;
      }

      // 종합 점수 (100점 만점)
      // 완료율: 35점, 정시제출률: 20점, RT과제: 15점, 영작정답률: 15점, 제출시간: 15점
      const compositeScore = Math.round(
        (completionRate / 100) * 35 +
        (onTimeRate / 100) * 20 +
        (rtRate / 100) * 15 +
        (writingTotal > 0 ? (writingAccuracy / 100) * 15 : 0) +
        timeScore
      );
    ...
    // 반별(학교+학년)로 그룹핑 후 Top 2 선정
    const gradeGroups: Record<string, ScholarshipStudent[]> = {};
    scored.forEach(s => {
      const key = `${s.school}|||${s.grade}`;
      if (!gradeGroups[key]) gradeGroups[key] = [];
      gradeGroups[key].push(s);
    });

    const result: { school: string; grade: string; schoolLogoUrl?: string; scholars: ScholarshipStudent[] }[] = [];
    Object.entries(gradeGroups).forEach(([key, students]) => {
      const [school, grade] = key.split("|||");
      const sorted = students.sort((a, b) => b.compositeScore - a.compositeScore);
      const scholars = sorted.slice(0, 2).filter(s => s.compositeScore > 0);
      if (scholars.length > 0) {
        result.push({ school, grade, schoolLogoUrl: scholars[0].schoolLogoUrl, scholars });
      }
    });

    return result.sort((a, b) => a.school.localeCompare(b.school) || a.grade.localeCompare(b.grade));
  }, [allStudentStats, rtData, writingData, submitTimeData]);

  const month = new Date().getMonth() + 1;

  return (
    <Card className="overflow-hidden border-0 shadow-lg bg-gradient-to-br from-card via-card to-muted/30">
      <CardHeader className="pb-2 bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-500 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-amber-100">
              완료율 · 정시제출 · 리뷰과제 · 영작정답률 · 제출시간 종합 평가 · 반별 2명 선정
            </CardDescription>
          </div>
      ...
      <CardContent className="pt-4">
        {isLoadingStats ? (
        ...
        ) : scholarshipByGrade.length > 0 ? (
          <div className="space-y-4">
            {scholarshipByGrade.map(({ school, grade, schoolLogoUrl, scholars }) => (
              <div key={`${school}-${grade}`} className="rounded-xl border border-border/50 overflow-hidden">
                {/* 반 헤더 */}
                <div className="flex items-center gap-2 px-4 py-2.5 bg-muted/40 border-b border-border/30">
                  {schoolLogoUrl ? (
                    <img src={cacheBustUrl(schoolLogoUrl)} alt={school} className="w-5 h-5 rounded-full object-cover border border-border/50" />
                  ) : (
                    <div className="w-5 h-5 rounded-full bg-muted flex items-center justify-center text-[8px] font-bold text-muted-foreground border border-border/50">
                      {school.charAt(0)}
                    </div>
                  )}
                  <span className="text-sm font-semibold text-foreground">
                    {school.replace("고등학교", "고").replace("중학교", "중")} · {grade}
                  </span>
                </div>
                {/* 장학생 카드 */}
                <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border/30">
                  {scholars.map((scholar, idx) => (
                    <div
                      ...
                      onClick={() => onStudentClick(scholar as any)}
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground">{idx === 0 ? "🥇 1등 장학생" : "🥈 2등 장학생"}</p>
                        </div>
                        <span className="ml-auto text-lg font-black text-primary">{scholar.compositeScore}<span className="text-xs font-normal text-muted-foreground">점</span></span>
                      </div>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">완료율</span>
                          <span className={`font-semibold ${scholar.completionRate >= 80 ? "text-emerald-600" : "text-foreground"}`}>{scholar.completionRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">정시제출</span>
                          <span className={`font-semibold ${scholar.onTimeRate >= 80 ? "text-emerald-600" : "text-foreground"}`}>{scholar.onTimeRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">리뷰과제</span>
                          <span className="font-semibold">{scholar.rtSubmissionCount}/{scholar.rtTotalCount}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">영작정답</span>
                          <span className={`font-semibold ${scholar.writingAccuracy >= 80 ? "text-emerald-600" : "text-foreground"}`}>{scholar.writingAccuracy}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
            ...
            <span className="text-sm">아직 데이터가 부족합니다</span>
          </div>
        )}
        ...
        <div className="mt-4 pt-3 border-t border-border/40">
          <p className="text-[10px] text-muted-foreground">
            📊 종합점수 = 완료율(35점) + 정시제출률(20점) + 리뷰과제(15점) + 영작정답률(15점) + 제출시간(15점)
          </p>
        </div>
      </CardContent>
    </Card>
...
export default function Statistics() {
  ...
  // 2026년 3월은 3월 9일부터 계산
  const isMarc2026 = today.getFullYear() === 2026 && today.getMonth() === 2;
  const startOfMonthStr = isMarc2026 ? "2026-03-09" : startOfMonth.toISOString().split('T')[0];
  ...
  const { data: schoolChartData = [], isLoading: isLoadingSchoolChart } = useQuery({
    queryKey: ["school-chart-data-30d", todayStr, thirtyDaysAgoStr, ownerCodeId],
    enabled: studentIdsReady,
    queryFn: async () => {
      let schoolsQuery = supabase.from("schools").select("id, name, logo_url").order("name");
      ...
      const chartData = await Promise.all(
        schools.map(async (school) => {
          const { data: grades } = await supabase.from("grades").select("id").eq("school_id", school.id);
          const gradeIds = grades?.map(g => g.id) || [];
          if (gradeIds.length === 0) return { school: school.name, 완료: 0, 미완료: 0, shortName: school.name.replace("고등학교", "고").replace("중학교", "중"), logoUrl: school.logo_url };

          const { data: studentsList } = await supabase.from("students").select("id, created_at").in("grade_id", gradeIds);
          const studentIdList = studentsList?.map(s => s.id) || [];
          const totalStudents = studentIdList.length;
          if (totalStudents === 0) return { school: school.name, 완료: 0, 미완료: 0, shortName: school.name.replace("고등학교", "고").replace("중학교", "중"), logoUrl: school.logo_url };

          // 학생별 실제 기대일수 합산
          const SYSTEM_START = "2026-02-08";
          let totalExpected = 0;
          (studentsList || []).forEach((s: any) => {
            const createdAtKST = new Date(new Date(s.created_at).getTime() + 9 * 60 * 60 * 1000);
            const createdDateStr = createdAtKST.toISOString().split('T')[0];
          ...
          const shortName = school.name.replace("고등학교", "고").replace("중학교", "중");
          return { school: school.name, shortName, 완료: totalSubmitted, 미완료: Math.max(0, totalExpected - totalSubmitted), logoUrl: school.logo_url };
        })
      );
    ...
    queryFn: async () => {
      ...
      const stats = await Promise.all(
        students.map(async (student: any) => {
          // 학생의 실제 시작일 계산
          const createdAtKST = new Date(new Date(student.created_at).getTime() + 9 * 60 * 60 * 1000);
          const createdDateStr = createdAtKST.toISOString().split('T')[0];
    ...
    queryFn: async () => {
      ...
      const studentStats = await Promise.all(
        students.map(async (student: any) => {
          ...
          if (effectiveStart > todayStr) {
            return { id: student.id, name: student.name, school: student.grade?.school?.name || "", grade: student.grade?.name || "", rate: 0, submissionCount: 0, lateCount: 0, missedCount: 0, schoolLogoUrl: student.grade?.school?.logo_url || undefined };
          }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" className="bg-white/10 border-white/10 text-white hover:bg-white/20" onClick={() => setBulkReportOpen(true)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* 학교별 완료/미완료 현황 */}
        <Card className="overflow-hidden border-0 shadow-lg bg-gradient-to-br from-card via-card to-muted/30">
          <CardHeader className="pb-2 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 text-white">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg font-semibold text-white">학교별 제출 현황</CardTitle>
          ...
          <CardContent className="pt-6">
            {isLoadingSchoolChart ? (
              ...
              <div className="space-y-5">
                {schoolChartData.map((school) => {
                  const total = school.완료 + school.미완료;
                  const rate = total > 0 ? Math.round((school.완료 / total) * 100) : 0;
                  return (
                    <div key={school.school} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {school.logoUrl ? (
                            <img src={cacheBustUrl(school.logoUrl)} alt={school.shortName} className="w-6 h-6 rounded-full object-cover border border-border" />
                          ) : (
                          ...
                          <span className="text-sm font-semibold">{school.shortName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-muted-foreground">
                            <span className="text-emerald-600 font-bold">{school.완료}건</span>
      ...
      <ScholarshipSection
        allStudentStats={allStudentStats}
        effectiveStudentIds={effectiveStudentIds}
        studentIdsReady={studentIdsReady}
      ...
      />
      <BulkMonthlyReportDialog
        open={bulkReportOpen}
        onOpenChange={setBulkReportOpen}
      />
    </div>
  );
}
