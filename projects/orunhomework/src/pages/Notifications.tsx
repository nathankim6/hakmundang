...
const submissionTypeLabels: Record<string, {label: string;className: string;}> = {
  ...
  review: { label: "리뷰 과제", className: "bg-purple-100 text-purple-700" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 p-4 border border-blue-100/60 shadow-sm group hover:shadow-md transition-all">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 p-4 border border-emerald-100/60 shadow-sm group hover:shadow-md transition-all">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-rose-50 to-pink-50 p-4 border border-rose-100/60 shadow-sm group hover:shadow-md transition-all">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-50 to-purple-50 p-4 border border-violet-100/60 shadow-sm group hover:shadow-md transition-all">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={typeFilter} onValueChange={(v) => handleFilterChange(setTypeFilter, v)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="review">리뷰 과제</SelectItem>
          ...
          <>
             ...
             <div className="divide-y divide-border/20">
              {paginatedNotifications.map((notification, index) => {
                ...
                const isSMS = messageLength <= 45;
                const messageType = isSMS ? "SMS" : "LMS";
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      ...
                      title="클릭하여 전체 내용 보기"
                      className="text-[11px] text-muted-foreground/70 truncate cursor-pointer hover:text-foreground hover:underline underline-offset-2"
                    >
                    ...
                    {submissionType ? (
                      <span className={`text-[8px] font-bold px-1 py-0.5 rounded text-center ${submissionType.className}`}>
                        {submissionType.label === "일일 단어" ? "단어" : submissionType.label === "리뷰 과제" ? "리뷰" : "수동"}
                      </span>
                    ) : <span />}
      ...
      <Dialog open={!!detailNotification} onOpenChange={(o) => !o && setDetailNotification(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {detailNotification?.student?.name || "학생"} ·{" "}
          ...
          {detailNotification?.error_message && (
            <p className="text-xs text-destructive">{detailNotification.error_message}</p>
          )}
          <div className="flex justify-end gap-2">
            <Button
              ...
              size="sm"
              onClick={() => {
                navigator.clipboard.writeText(detailNotification?.message || "");
                toast.success("메시지를 복사했습니다.");
              }}
            >
              복사
            </Button>
            <Button size="sm" onClick={() => setDetailNotification(null)}>닫기</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
