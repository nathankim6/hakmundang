...
function renderHighlightedText(fullText: string, partialText: string, type: "english" | "korean") {
  ...
  if (regions.length === 0) return fullText;

  const elements: React.ReactNode[] = [];
  let cursor = 0;
  regions.forEach((r, i) => {
    if (cursor < r.start) {
      elements.push(<span key={`gap-${i}`} className="text-muted-foreground/40">{fullText.substring(cursor, r.start)}</span>);
    }
    elements.push(<span key={`hl-${i}`} className="text-primary font-semibold bg-primary/5 px-0.5 rounded">{r.text}</span>);
    cursor = r.end;
  });
  if (cursor < fullText.length) {
    elements.push(<span key="tail" className="text-muted-foreground/40">{fullText.substring(cursor)}</span>);
  }
  return <>{elements}</>;
}

export function WritingPractice({ gradeId, schoolId, onClose }: WritingPracticeProps) {
  const { data: writingSentences = [], isLoading } = useQuery({
    queryKey: ["writing-practice-sentences", gradeId, schoolId],
    queryFn: async () => {
      let query = supabase
        .from("passages")
        .select("id, title, sentences, korean_content");
      if (schoolId) {
        query = query.or(`grade_id.eq.${gradeId},and(grade_id.is.null,school_id.eq.${schoolId})`);
      } else {
        query = query.eq("grade_id", gradeId);
      }
      const { data: passages, error: pErr } = await query;
      if (pErr) throw pErr;
      ...
      return sorted.map((ws) => {
        ...
        // Support multi-part partial via ||| delimiter
        const isMultiPart = ws.english_sentence.includes("|||");
        const isPartial = isMultiPart || (ws.english_sentence.trim() !== fullEnglish.trim() && fullEnglish.includes(ws.english_sentence.trim()));
        const isMultiPartKorean = korean.includes("|||");
        const isPartialKorean = isMultiPartKorean || (korean !== fullKorean && fullKorean.includes(korean));
        return {
          ...ws,
  ...
  // Combine words from all ||| parts for quiz
  const getQuizText = (text: string) => text.includes("|||") ? text.split("|||").map(p => p.trim()).filter(Boolean).join(" ") : text;
  const correctWords = useMemo(
    () => getQuizText(currentSentence?.english_sentence || "").split(" ").map(stripPunctuation).filter(Boolean) || [],
    [currentSentence?.id]
  );

  useEffect(() => {
    if (!currentSentence) return;
    const quizText = getQuizText(currentSentence.english_sentence);
    const words = quizText.split(" ").map(stripPunctuation).filter(Boolean);
        ...
        <div className={`rounded-xl border overflow-hidden transition-all duration-300 ${
          ...
          <div className={`px-5 py-4 transition-colors duration-300 ${
            ...
            <div className="flex items-center gap-2 mb-2.5">
              ...
              {currentSentence.passage_title && (
                <span className="ml-auto text-[10px] text-muted-foreground/60 font-medium bg-muted/50 px-2 py-0.5 rounded-full truncate max-w-[140px]" title={currentSentence.passage_title}>
                  📄 {currentSentence.passage_title}
                </span>
              )}
            </div>
              ...
              {currentSentence.is_partial && currentSentence.full_english ? (
                <>
                  {(() => {
                    ...
                    const raw = currentSentence.english_sentence.trim();
                    const isMulti = raw.includes("|||");
                    
                    // For multi-part, compute prefix before first part & suffix after last part
                    const parts = isMulti ? raw.split("|||").map(p => p.trim()).filter(Boolean) : [raw];
                    const regions = parts.map(p => {
                      const idx = full.indexOf(p);
                      return idx !== -1 ? { start: idx, end: idx + p.length } : null;
                    }).filter(Boolean).sort((a, b) => a!.start - b!.start) as { start: number; end: number }[];
                    
                    const prefix = regions.length > 0 && regions[0].start > 0 ? full.substring(0, regions[0].start).trim() : "";
                    const suffix = regions.length > 0 ? full.substring(regions[regions.length - 1].end).trim() : "";
                    // Middle gaps (between selected parts)
                    const middleGaps: string[] = [];
                    for (let i = 0; i < regions.length - 1; i++) {
                      const gap = full.substring(regions[i].end, regions[i + 1].start).trim();
                      if (gap) middleGaps.push(gap);
                    }
