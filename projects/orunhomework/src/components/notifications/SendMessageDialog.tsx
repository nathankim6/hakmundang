...
export function SendMessageDialog({ open, onOpenChange }: SendMessageDialogProps) {
  ...
  const { studentIds: ownerStudentIds, shouldFilter, isLoading: isLoadingOwner } = useOwnerStudentIds();
  ...
  const { data: students = [], isLoading } = useQuery({
    queryKey: ["students-for-messaging", ownerStudentIds],
    queryFn: async () => {
      let query = supabase
        .from("students")
        .select(`
      ...
      if (shouldFilter && ownerStudentIds) {
        if (ownerStudentIds.length === 0) return [];
        query = query.in("id", ownerStudentIds);
      }
