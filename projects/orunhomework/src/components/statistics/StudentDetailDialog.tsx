              ...
              <div key={item.key} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-muted/30 border border-border/30">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    ({new Date(s.submission_date).toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })})
                  </span>
                </div>
...
export default function StudentDetailDialog({ open, onOpenChange, student }: StudentDetailDialogProps) {
  ...
  // 해당 월의 dismissed 날짜 조회
  const { data: dismissedDatesForMonth = [] } = useQuery({
    queryKey: ["student-dismissed-dates", student?.id, selectedMonth],
    queryFn: async () => {
      if (!student) return [];
      const { data } = await supabase
        ...
        .eq("student_id", student.id)
        .gte("dismissed_date", format(monthStart, 'yyyy-MM-dd'))
        .lte("dismissed_date", format(monthEnd, 'yyyy-MM-dd'));
      return (data || []).map(d => d.dismissed_date);
    },
    enabled: !!student && open,
  });
  ...
  const { data: rtSubmissions, isLoading: isLoadingRT } = useQuery({
    ...
    queryFn: async () => {
      ...
      const submitted = data?.filter(s => s.status === "submitted" || s.status === "reviewed" || s.status === "completed").length || 0;
  ...
  // Note: Pause state not checked here as this is a statistics view showing historical data
  ...
  const { data: extendedDeadlines = [] } = useQuery({
    queryKey: ["student-detail-extensions", student?.id],
    queryFn: async () => {
      if (!student) return [];
      const { data } = await supabase
  ...
  // 리뷰 과제 상세 (pending/submitted 구분용)
  ...
  const overdueWithoutExtension = missedDailyWords.filter(d => !extendedDailyWordDates.has(d));
  const overdueWithExtension = missedDailyWords.filter(d => extendedDailyWordDates.has(d));
  ...
  // dismissed dates set (monthlyStats에서도 사용하므로 먼저 선언)
  const dismissedDateSet = new Set(dismissedDatesForMonth);

  // 학생의 과제 시작일 계산 (monthlyStats보다 먼저 선언해야 TDZ 에러 방지)
  ...
  function getAssignmentStartDate() {
  ...
  const assignmentStartDate = getAssignmentStartDate();
  ...
  const monthlyStats = (() => {
    ...
    const validDays = daysInMonth.filter(d => {
      ...
      if (dismissedDateSet.has(dayStr)) return false;
      if (dayStart < assignmentStartDate) return false;
      return true;
    });
                      ...
                      <div className="space-y-1.5">
                        ...
                        {!todaySubmitted && (
                          <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-muted/30 border border-border/30">
                            ...
                            <div className="flex-1 min-w-0 flex items-center gap-1.5">
                              <span className="font-medium text-foreground text-xs">일일 단어</span>
                              <span className="text-[10px] text-muted-foreground">({new Date().toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })})</span>
                            </div>
                        ...
                        {/* 기한연장된 단어과제 */}
                        {overdueWithExtension.map(date => {
                          const ext = extendedDeadlines.find((e: any) => e.daily_word_date === date);
                          return (
                            <div key={`ext-${date}`} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-muted/30 border border-amber-500/20">
                              <div className="w-7 h-7 rounded-md bg-slate-900 flex items-center justify-center flex-shrink-0">
                                <Camera className="w-3.5 h-3.5 text-white" />
                              </div>
                              <div className="flex-1 min-w-0 flex items-center gap-1.5">
                                ...
                                {ext && <span className="text-[9px] text-muted-foreground">~{new Date(ext.new_due_date).toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })}</span>}
                              </div>
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-500/15 text-amber-600">기한연장</span>
                            </div>
                          );
                        })}
                        {/* 리뷰 과제 미제출 */}
                          ...
                          return (
                            <div key={s.id} className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-muted/30 border ${isOverdue ? 'border-destructive/20' : 'border-border/30'}`}>
                              ...
                              <div className="flex-1 min-w-0 flex items-center gap-1.5">
                                <span className="font-medium text-foreground text-xs truncate max-w-[120px]">{s.homework?.title}</span>
                                <span className="text-[10px] text-muted-foreground whitespace-nowrap">
                                  ~{new Date(s.homework?.due_date).toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })}
                                </span>
                              </div>
              ...
              <Calendar
                ...
                modifiers={{
                  ...
                  missed: calendarDays.filter(day => {
                    ...
                    return !submissionDates.has(dayStr) && !dismissedDateSet.has(dayStr) && !isFuture(day) && !isSameDay(day, today) && dayStart >= assignmentStartDate;
                  }),
                  beforeStart: calendarDays.filter(day => {
                    const dayStr = format(day, 'yyyy-MM-dd');
                    const dayStart = new Date(day.getFullYear(), day.getMonth(), day.getDate());
                    return !isFuture(day) && !isSameDay(day, today) && (dayStart < assignmentStartDate || dismissedDateSet.has(dayStr));
                  }),
                }}
                modifiersStyles={{
                  submitted: {
            ...
            {rtSubmissions && rtSubmissions.total > 0 && (
              <div className="space-y-2.5 p-4 rounded-xl bg-gradient-to-r from-muted/40 via-muted/20 to-muted/40 border border-border/50 mb-6">
                <div className="flex justify-between text-xs">
                  <span className="font-semibold text-foreground/80">리뷰 과제 완료율</span>
