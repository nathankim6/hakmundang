...
const DEFAULT_TEMPLATES: MessageTemplates = {
  ...
  reviewTaskReview: "{studentName} 학생의 녹음 과제 검토가 완료되었습니다.",
};
...
export async function saveMessageTemplates(templates: MessageTemplates, ownerCodeId?: string | null): Promise<void> {
  ...
  let existQuery = supabase
    ...
    .eq("key", SETTINGS_KEY);
  if (ownerCodeId) {
    existQuery = existQuery.eq("owner_code_id", ownerCodeId);
  } else {
    existQuery = existQuery.is("owner_code_id", null);
  }
  ...
  if (existing) {
    ...
    const { error } = await supabase
      .from("app_settings")
      .insert({ key: SETTINGS_KEY, value: JSON.stringify(templates), owner_code_id: ownerCodeId || null });
    if (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="예: {studentName} 학생의 녹음 과제 검토가 완료되었습니다."
              className="min-h-[80px]"
            />
            <p className="text-xs text-muted-foreground">
