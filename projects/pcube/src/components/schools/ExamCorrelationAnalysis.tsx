...
import { TrendingUp, TrendingDown, BarChart3, Target, Minus, User } from "lucide-react";
...
export default function ExamCorrelationAnalysis() {
  ...
  // Fetch all students with grade info
  ...
  // Fetch mock exam scores for the year
  ...
  // Fetch daily word submissions for the year
  ...
  // Calculate correlation data per student
  const correlationData = useMemo(() => {
    if (allStudents.length === 0) return [];

    // Build submission count per student (total days submitted)
    const subCountMap: Record<string, number> = {};
    dailySubmissions.forEach((s: any) => {
      subCountMap[s.student_id] = (subCountMap[s.student_id] || 0) + 1;
    });

    // Build avg score per student
    ...
    // Get the student's grade/school info
    const gradeMap = new Map(allGrades.map(g => [g.id, g]));
    const schoolMap = new Map(schools.map(s => [s.id, s]));

    return allStudents
      .filter(st => scoreAccum[st.id]) // Only students with exam scores
      .map(st => {
  ...
  // (monthlyTrend is defined after summary below)

  // Summary stats - top/mid/bottom 20%
  const summary = useMemo(() => {
    ...
    const midCount = Math.max(1, top20Count); // same size as top for fairness
  ...
  // Monthly trend data - for top 20% students only
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <>
          {/* Summary cards - Top/Mid/Bottom 20% */}
          {summary && (
          ...
          {/* Scatter: submission count vs avg score */}
          <Card>
            <CardHeader className="pb-2">
