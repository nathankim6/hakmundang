import { Settings as SettingsIcon, User, Phone, Key, Eye, EyeOff, MessageSquare, Loader2, Shield, ExternalLink, CheckCircle2, XCircle, Save, Instagram } from "lucide-react";
        ...
        data.forEach((setting) => {
          switch (setting.key) {
            ...
            case "instagram_url":
              ...
              break;
            case "instagram_label":
              setInstagramLabel(setting.value);
              break;
          }
        });
      }
      ...
      <Card className="shadow-sm">
        <CardContent className="pt-6 space-y-5">
          <SectionHeader 
            icon={Instagram} 
            title="인스타그램 링크" 
            description="학생 대시보드에 표시할 인스타그램 주소를 설정합니다"
            statusBadge={
              instagramUrl ? (
                <Badge variant="secondary" className="text-[10px] gap-1 bg-emerald-50 text-emerald-700 border-emerald-200">
                  <CheckCircle2 className="w-3 h-3" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="instagram-url" className="text-xs font-medium text-muted-foreground">인스타그램 URL</Label>
              <Input
                id="instagram-url"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="instagram-label" className="text-xs font-medium text-muted-foreground">표시 이름</Label>
              <Input
                id="instagram-label"
                placeholder="YG Instagram"
          ...
          <div className="flex items-center justify-between pt-1">
            <p className="text-[11px] text-muted-foreground">
              비워두면 학생 화면에 인스타 아이콘이 표시되지 않습니다
            </p>
            <Button size="sm" onClick={async () => {
              setIsSavingInstagram(true);
              try {
                await upsertSetting("instagram_url", instagramUrl);
                await upsertSetting("instagram_label", instagramLabel);
                toast.success("인스타그램 설정이 저장되었습니다.");
              } catch {
                toast.error("저장에 실패했습니다.");
              } finally {
                setIsSavingInstagram(false);
              }
            }} disabled={isSavingInstagram} className="h-8 px-4 text-xs">
              {isSavingInstagram ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
              저장
            </Button>
          </div>
        </CardContent>
