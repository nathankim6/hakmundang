...
const ApiKeyDialog: React.FC<ApiKeyDialogProps> = ({
  ...
  const testApiConnection = async () => {
    try {
      ...
      const response = await fetch('https://jpanpwbdlhsxnyaldddm.supabase.co/functions/v1/generate-understanding', {
        method: 'POST',
        headers: {
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogTrigger asChild>
        <Button variant={isApiConnected ? "outline" : "secondary"} size="sm" className={cn("transition-all duration-200 w-full", isApiConnected ? "text-green-600 hover:text-green-700 hover:bg-green-50 border-green-200" : "text-amber-600 hover:text-amber-700 hover:bg-amber-50")}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            이해 자료 생성 기능을 사용하기 위해 OpenAI API 키를 입력해주세요.
          </DialogDescription>
        </DialogHeader>
