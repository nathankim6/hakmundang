...
export const TypeButton = ({ type, isSelected, hasAccess, onClick, logos }: TypeButtonProps) => {
  const handleClick = () => {
    ...
    if (type.id === "sungReference") {
      window.open("https://chatgpt.com/g/g-6779f42e26208191bcbfffeedd80d875-sungyiyeogo-jicingcuron", "_blank");
      return;
    }
    if (type.id === "sungExternal") {
      window.open("https://chatgpt.com/g/g-67498d63f1048191a987654ac0a2bd44-sungyiyeogo1-oebujimun", "_blank");
      return;
    }
    ...
    if (type.id === "fourKings") {
      window.open("https://chatgpt.com/g/g-NGhQW1fEq-olheunyeongeo-4daeceonwang-munjejejag", "_blank");
      return;
    }
  ...
  return (
    ...
    >
      <span className="relative z-10 flex items-center gap-2">
        ...
        {!isSelected && hasAccess && type.id !== "dangDict" && type.id !== "summaryBlank" && type.id !== "sungReference" && type.id !== "sungExternal" && type.id !== "sungnamVocab1" && type.id !== "sungnamVocab2" && type.id !== "illustration" && type.id !== "fourKings" && type.id !== "orderWriting" && (
          <Sparkles 
