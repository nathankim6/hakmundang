      ...
      for (const typeEntry of updatedTypes) {
        ...
        for (const passage of validPassages) {
          try {
            ...
            console.log(`독립적으로 문제 생성 시작 - 유형: [${typeEntry.type.name}] ${typeEntry.type.id}, 지문 ID: ${passage.id}`);
            
            // Generate question for single passage
            const result = await generateQuestion(typeEntry.type, passage.text, difficulty);
            console.log(`문제 생성 완료 - 지문 ID: ${passage.id}`);
            ...
            toast({
              title: "문제 생성 완료",
              description: `${currentQuestion}번째 문제가 생성되었습니다.`,
            });
