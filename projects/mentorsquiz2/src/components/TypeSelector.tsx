...
export const TypeSelector = ({ selectedTypes, onSelect, onRemove }: TypeSelectorProps) => {
  ...
  const schoolTypes = types.filter(type => 
    type.id.match(/^(dictionary|collocation|exampleSentences|reference|contentInference|referenceInference|conjunction|trueOrFalse|sungnamVocab|sungExternal|sungReference|yeongExternal|dangDict)/)
  );
  
  // 서답형
  const writingTypes = types.filter(type => 
    type.id.match(/^(orderWritingKorean|orderWriting|summaryBlank|summaryVocab|topicWriting)$/)
  );
  
  // 워크북 제작 - removed sentenceSplitter and illustration
  const workbookTypes = types.filter(type => 
    type.id.match(/^(logicFlow|grammarWorkbook|vocabWorkbook|orderBlankWorkbook|blankWorkbook)$/)
  );
  
  // 기타 콘텐츠
  const contentTypes = types.filter(type => 
    type.id.match(/^(synonymAntonym|weekendClinic|fourKings)$/)
  );
