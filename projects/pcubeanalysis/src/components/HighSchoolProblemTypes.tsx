    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {(() => {
            // Compute separate per-type indices: objective starts at 1, subjective starts at 1
            let objCount = 0;
            let subCount = 0;
            const indexMap = new Map<string, number>();
            problemTypes.forEach(t => {
              if (t.questionType === 'subjective') {
                subCount += 1;
                indexMap.set(t.id, subCount);
              } else {
                objCount += 1;
                indexMap.set(t.id, objCount);
              }
            });
            return problemTypes.map((type) => {
              const displayNum = indexMap.get(type.id) ?? 0;
              const isSub = type.questionType === 'subjective';
              return (
            <div 
              key={type.id} 
            ...
            >
              <div
                className={`flex-none w-12 h-8 rounded-lg flex items-center justify-center text-white font-medium text-xs shadow-inner ${
                  isSub
