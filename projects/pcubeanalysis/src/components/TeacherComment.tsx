...
const TeacherComment: React.FC<TeacherCommentProps> = ({
  ...
  return <Card className="p-6 my-4 shadow-lg bg-gradient-to-br border-0 relative overflow-hidden transform transition-all duration-300 hover:translate-y-[-3px]" style={{
    ...
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
      {teacherPhoto ? <div className="relative w-full md:w-40 h-40 flex-shrink-0 overflow-hidden rounded-xl shadow-lg border-0 transform transition-all duration-300 group-hover:scale-105" style={{
        boxShadow: `0 10px 30px -5px ${themeColors.light}70`
      }}>
          <div className="absolute inset-0 rounded-xl opacity-60 pointer-events-none" style={{
            background: `linear-gradient(135deg, ${themeColors.vibrant}, ${themeColors.highlight})`
          }} />
          <img src={teacherPhoto} alt={`${teacher} 선생님`} className="relative z-10 w-full h-full object-cover rounded-xl" />
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3 rounded-b-xl">
            <AutoResizeText text={`${teacher} 선생님`} maxFontSize={14} minFontSize={10} containerClassName="w-full" textClassName="text-white font-medium drop-shadow-md" />
          </div>
        </div> : <div className="relative w-full md:w-40 h-40 flex-shrink-0 overflow-hidden rounded-xl shadow-lg border-0 transform transition-all duration-300 group-hover:scale-105 bg-gray-100 flex justify-center items-center" style={{
        boxShadow: `0 10px 30px -5px ${themeColors.light}70`
      }}>
          ...
          <ScrollArea className="max-h-full pr-2">
            ...
            <div className="space-y-4 pl-4 relative text-justify pr-2">
              {evaluationCategories.map((category, index) => <div key={index} className="mb-3">
                ...
                <p className="text-gray-700 whitespace-pre-wrap leading-relaxed text-xl font-inter">
