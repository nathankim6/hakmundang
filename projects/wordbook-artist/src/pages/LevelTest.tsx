...
const VOCA_IDS: Record<string, string> = {
  'ORUN VOCA 0': 'a62f68e4-ef78-416f-9dec-896dea9ab6cd',
  'ORUN VOCA 1': '2b29919e-dcb0-4dd1-9228-85ba51a44237',
  'ORUN VOCA 2': '9ab3245c-9233-4b0f-a732-5dec6d11a67c',
    ...
    <div className="page-b5 shadow-2xl flex flex-col relative overflow-hidden" style={{
      ...
      <div className="flex-shrink-0 h-12 flex items-center justify-between relative overflow-hidden mt-5 rounded-lg" style={{
        ...
        <div className="flex items-center gap-2 relative z-10 pl-4">
          ...
          <span style={{ fontFamily: '"Orbitron", "Playfair Display", serif', fontWeight: 700, fontSize: '10px', color: '#FFFFFF', letterSpacing: '0.1em' }}>LEVEL TEST</span>
        </div>
      ...
      <div className="flex-1 py-2 px-1" style={{ marginLeft: leftMargin, marginRight: rightMargin }}>
        <div className={`grid ${sections.length > 1 ? 'grid-cols-2' : 'grid-cols-1 max-w-[50%]'} gap-4 h-full`}>
          {[0, 1].filter(c => sections[c]).map(colIdx => {
      ...
      <div className="flex-shrink-0 mb-4 relative" style={{ marginLeft: leftMargin, marginRight: rightMargin }}>
        ...
        <div className={`flex items-center h-8 ${isLeftPage ? 'justify-start' : 'justify-end'}`}>
          <div className="flex items-center gap-3 px-2">
            ...
            <span className="text-xs font-medium" style={{ color: '#888', fontFamily: '"Noto Sans KR", sans-serif' }}>ORUN VOCA Level Test</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-muted-foreground">VOCA 0~8에서 레벨테스트 단어를 추출하는 중...</p>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b no-print">
        <div className="container mx-auto flex items-center justify-between py-3 px-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-semibold">ORUN VOCA 0~8 레벨 테스트</h1>
          </div>
          <div className="flex items-center gap-2">
