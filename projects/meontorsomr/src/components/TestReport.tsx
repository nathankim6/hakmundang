...
const TestReport: React.FC<TestReportProps> = ({
  ...
  const results = useMemo(() => {
    const sectionResults = SECTIONS.map(section => {
      ...
      section.range.forEach(questionNumber => {
        const isCorrect = studentAnswers[questionNumber]?.answer === correctAnswers[questionNumber]?.answer;
      ...
      const allSectionScores = allResults.map(result => {
        let score = 0;
        section.range.forEach(questionNumber => {
          const isCorrect = result.student_answers[questionNumber]?.answer === correctAnswers[questionNumber]?.answer;
          if (isCorrect) {
      ...
      const sectionAvg = allSectionScores.length > 0 ? Math.round(allSectionScores.reduce((a, b) => a + b, 0) / allSectionScores.length * 10) / 10 : 0;
    ...
    const wrongAnswerRates = Array.from({
      ...
      const wrongCount = allResults.filter(result => result.student_answers[questionNumber]?.answer !== correctAnswers[questionNumber]?.answer).length;
      const rate = wrongCount / allResults.length * 100;
      return {
        questionNumber,
        rate,
        isCorrect: studentAnswers[questionNumber]?.answer === correctAnswers[questionNumber]?.answer
      };
    });
  ...
  return <Card className="w-full overflow-hidden border-0 shadow-lg">
          ...
          <div className="relative z-10 flex-1">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-6">
              <div className="flex items-center">
                <div className="relative">
                  <div className="relative p-1 bg-white backdrop-blur-sm rounded-full shadow-sm">
                    <img src="/lovable-uploads/d8f3c6d5-b3b8-4941-8d20-5dbf79e41f61.png" alt="Mentors Table Logo" className="h-14 w-auto rounded-full" />
                  </div>
                </div>
                <div className="ml-6">
                  <h1 className="font-bold text-3xl text-amber-900">전문가집단 영어학원 시험성적 리포트</h1>
                </div>
              </div>
              ...
              <div className="hidden md:flex items-center space-x-1 px-3 py-1 rounded-full bg-white border border-amber-200/50 text-amber-800 text-sm">
                ...
                <span>Mentors Table CSAT Score Report</span>
              </div>
            </div>
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  left: 30
                    ...
                    <Radar name="점수" dataKey="score" stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.4} strokeWidth={2} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={scoreComparisonData}>
                    ...
                    <Bar name="응시자 평균" dataKey="average" fill="#94A3B8" radius={[4, 4, 0, 0]} />
                    <Bar name="내 점수" dataKey="score" fill="#3B82F6" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg">
            ...
            <div className="p-6">
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50 hover:bg-slate-50/80">
                    <TableHead className="font-semibold text-sm whitespace-nowrap text-slate-700">순위</TableHead>
                ...
                <TableBody>
                  {results.total.wrongAnswerTop3.map((item, index) => <TableRow key={index} className="hover:bg-blue-50/30 transition-colors border-b border-gray-100">
                      <TableCell className="text-sm whitespace-nowrap font-medium">
                        <div className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-slate-100 text-slate-700 text-xs font-semibold">
                          {index + 1}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        {item.isCorrect ? <div className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-blue-100 text-blue-600 font-bold text-lg">
                            O
                          </div> : <div className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-red-100 text-red-600 font-bold text-lg">
                            X
                          </div>}
                      </TableCell>
                    </TableRow>)}
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-md overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-5 sm:grid-cols-9 gap-2">
              ...
              return <div key={questionNumber} className={`p-2 rounded-lg border ${isCorrect ? 'bg-blue-50 border-blue-200 hover:bg-blue-100' : 'bg-red-50 border-red-200 hover:bg-red-100'} transition-colors`}>
                    <div className="flex justify-between items-center text-xs mb-1">
                      <span className="font-medium">{questionNumber}</span>
                    </div>
                    <div className="text-center space-y-1">
                      {isCorrect ? <div className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-blue-100 text-blue-600 font-bold text-sm">
                          O
                        </div> : <div className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-red-100 text-red-600 font-bold text-sm">
                          X
                        </div>}
