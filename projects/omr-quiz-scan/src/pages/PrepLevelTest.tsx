...
import { getPrepSet, normalizePrepVersion, PREP_VERSION_META } from '@/data/prepVersions';
...
const PrepLevelTest = () => {
  ...
  const prepVersion = normalizePrepVersion(searchParams.get('version'));
  const prepSet = getPrepSet(prepVersion);
  const allPrepQuestions = prepSet.questions;
  const prepAnalysisCategories = prepSet.analysisCategories;
  const prepSectionNames = prepSet.sectionNames;
  const getPrepQuestionPoints = prepSet.getPoints;
  const calculatePrepTotalMaxScore = () => prepSet.totalMaxScore;
  ...
  // 답안 저장 훅
  const { loadSavedState, saveState, clearSavedState, markAsLoaded } = useTestAnswerPersistence(`prep-${prepVersion}`);
  ...
  const {
    getDistractors
  } = usePrepVocabularyDistractors(allPrepQuestions);
  const sections: SectionType[] = (['reading', 'grammarA', 'grammarB', 'grammarC', 'vocabulary', 'sentenceAnalysis'] as SectionType[]).filter(sec => allPrepQuestions.some(q => (q.section === 'grammar' ? `grammar${q.grammarLevel || 'A'}` : q.section) === sec));
  ...
  // 버전별 실제 문항 수 기반 시험 구성 표시
  const sectionStructure = ([
    { key: 'reading', label: '독해' },
    { key: 'grammarA', label: '문법A' },
    { key: 'grammarB', label: '문법B' },
    { key: 'grammarC', label: '문법C' },
    { key: 'vocabulary', label: '어휘' },
    { key: 'sentenceAnalysis', label: '문장구조' },
  ] as { key: SectionType; label: string }[])
    .map(s => ({ label: s.label, count: allPrepQuestions.filter(q => getSectionFromQuestion(q) === s.key).length }))
    .filter(s => s.count > 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const {
        ...
        answers: { ...answers, __prepVersion: prepVersion } as any,
  ...
  // 시작 화면
  if (!isStarted) {
    const logoSrc = orunLogo;
    const titleText = 'Orun English Assessment Test';
    const subtitleText = `옳은영어 영어 성취도 진단 리포트 · ${PREP_VERSION_META[prepVersion].label}(${PREP_VERSION_META[prepVersion].subtitle})\u00A0`;
        ...
        <div className="max-w-lg mx-auto">
          <Card className="p-6 shadow-xl border-0 bg-white/80 backdrop-blur">
            <div className="text-center mb-6">
              <img src={logoSrc} alt="옳은영어 로고" className="w-24 h-24 mx-auto mb-4 object-contain" />
  ...
  return <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-3">
        {currentQuestion && <Card className="p-3 sm:p-4 shadow-lg border-0 bg-white/90 backdrop-blur relative">
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t">
              ...
              {currentSection === sections[sections.length - 1] && currentQuestionIndex === sectionQuestions.length - 1 ? <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700">
                  {isSubmitting ? <>
      ...
      {/* Footer */}
      <div className="text-center text-[10px] text-slate-400 py-6 border-t border-slate-100 mt-10">
