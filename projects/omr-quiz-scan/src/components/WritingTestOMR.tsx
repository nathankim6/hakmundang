...
const getClassList = (): string[] => {
  ...
  return ["1FO", "1INT", "1AD", "2FO", "2INT", "2AD", "3FO", "3INT", "3AD", "TOP", "고등부", "신규생", "IVY"];
};

const STORAGE_KEY_PREFIX = 'orun_writing_progress_';
import { hasAlreadySubmitted, markSubmitted } from "@/utils/submissionGuard";
...
const WritingTestOMR = () => {
  ...
  useEffect(() => {
    ...
    if (!studentName.trim() && !studentPhone.trim() && Object.keys(studentAnswers).length === 0 && !studentClass) {
      return;
    }
  ...
  }, [storageKey, studentAnswers, studentName, studentPhone, studentClass, shuffledWords]);
  ...
  const calculateScore = async () => {
    ...
    const submissionKey = `${studentName.trim()}${studentPhone.trim()}`;
    if (hasAlreadySubmitted(testId, submissionKey)) {
      toast({
        title: "이미 제출하였습니다",
        description: "동일한 시험은 한 번만 제출할 수 있습니다.",
        variant: "destructive"
      });
      clearSavedState();
      navigate('/');
    ...
    try {
      ...
      clearSavedState();
      markSubmitted(testId, submissionKey);
      ...
      navigate('/writing-result', {
        state: {
          ...
          results,
          testData,
          studentName: `${studentName.trim()}${studentPhone.trim()}`,
          ...
          <div className="flex gap-2 sm:grid sm:grid-cols-3 sm:gap-4">
            ...
            <div className="flex-[1.5] space-y-1">
              ...
              <Input
                ...
                className="h-9 sm:h-11 border-slate-200 bg-white hover:bg-slate-50 transition-colors rounded-lg sm:rounded-xl shadow-sm placeholder:text-slate-400 text-sm" placeholder="이름 (예: 김옳은)" />
            </div>
            <div className="flex-1 space-y-1">
              <label className="text-[10px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wider">번호 뒷 4자리</label>
              <Input
                value={studentPhone}
                inputMode="numeric"
                maxLength={4}
                onChange={(e) => setStudentPhone(e.target.value.replace(/\D/g, '').slice(0, 4))}
                className="h-9 sm:h-11 border-slate-200 bg-white hover:bg-slate-50 transition-colors rounded-lg sm:rounded-xl shadow-sm placeholder:text-slate-400 text-sm" placeholder="5554" />
              <p className="text-[9px] sm:text-[10px] text-red-500 font-semibold mt-0.5">⚠ 이름과 번호 뒷4자리 모두 입력해야 제출됩니다</p>
            </div>
          </div>
        </div>
