...
const TestActions = ({ testId, isEnded, onDelete, onEndTest, testData }: TestActionsProps) => {
  ...
  useEffect(() => {
    ...
    setIsAdmin(!!code);
    if (!code) { setIsVerifiedAdmin(false); return; }
    let cancelled = false;
    supabase.functions
      .invoke('verify-access-code', { body: { code } })
      .then(({ data, error }) => {
        if (!cancelled) setIsVerifiedAdmin(!error && !!data?.valid && !!data?.isAdmin);
      })
      .catch(() => { if (!cancelled) setIsVerifiedAdmin(false); });
  ...
  return (
    <div className="flex items-center gap-1.5 shrink-0">
      {/* 엑셀 일괄 결과 등록 (관리자 전용) */}
      {isVerifiedAdmin && !isWritingTest && <BulkResultUpload testData={testData} />}
