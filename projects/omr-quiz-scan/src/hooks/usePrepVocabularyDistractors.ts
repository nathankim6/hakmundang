...
import { prepVocabularyQuestions, type PrepLevelTestQuestion } from '@/data/prepLevelTestQuestions';
 ...
 */
export function usePrepVocabularyDistractors(questions?: PrepLevelTestQuestion[]) {
  // 버전별 어휘 문항(없으면 기본 v2)을 기준으로 고정 선지 맵 생성
  const source = questions && questions.length > 0
    ? questions.filter(q => q.section === 'vocabulary')
    : prepVocabularyQuestions;

  const distractorsMap = useMemo<Map<number, DistractorData>>(() => {
