  ...
  const loadInitialData = async () => {
    try {
      ...
      if (testsError) {
        ...
        // Filter out locally deleted tests
        let testsList = initialTests ? JSON.parse(JSON.stringify(initialTests)) : [];
        try {
          const deleted: string[] = JSON.parse(localStorage.getItem('deleted_tests') || '[]');
          if (Array.isArray(deleted) && deleted.length > 0) {
            testsList = testsList.filter((t: any) => !deleted.includes(t.test_id));
          }
        } catch {}
        _testsCache = testsList;
      ...
      if (testResultsError) {
        ...
        // Deep copy to avoid reference issues and filter out deleted
        let resultsList = initialTestResults ? JSON.parse(JSON.stringify(initialTestResults)) : [];
        try {
          const deleted: string[] = JSON.parse(localStorage.getItem('deleted_tests') || '[]');
          if (Array.isArray(deleted) && deleted.length > 0) {
            resultsList = resultsList.filter((r: any) => !deleted.includes(r.test_id));
          }
        } catch {}
        _testResultsCache = resultsList;
