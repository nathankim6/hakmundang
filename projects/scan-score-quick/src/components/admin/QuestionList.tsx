  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      extracted.forEach((item) => {
        ...
        const currentPoints = answers[num]?.points || 2;
              ...
              <span className={`text-xs px-2 py-1 rounded font-bold ${
                (() => {
                  ...
                  for (let i = 1; i <= questionCount; i++) {
                    total += answers[i]?.points || 2;
                  }
                  return total === 100 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600';
                })()
              }`}>
                현재 {(() => {
                  let total = 0;
            ...
            <div className="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  min="1"
                  max="10"
                  value={answers[num]?.points || 2}
                  onChange={(e) => handlePointsChange(num, parseInt(e.target.value) || 2)}
                  onFocus={(e) => e.target.select()}
