...
const AdminForm = () => {
  ...
  const handleAnswerChange = (questionNumber: number, answer: QuestionAnswer) => {
    // Ensure default points are set for new answers
    const answerWithPoints = {
      ...answer,
      points: answer.points || 2
    };
  ...
  const handleTestFormatChange = (format: TestFormat) => {
    ...
    // Initialize empty answers for all questions with default points
    for (let i = 1; i <= questionCount; i++) {
      newAnswers[i] = {
        type: 'multiple',
        answer: [], // Empty answer - no pre-selection
        points: 2 // Default 2 points for each question
      };
    }
  ...
  const handleQuestionCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Add default points for new questions
    for (let i = 1; i <= newCount; i++) {
      ...
      } else if (!newAnswers[i].points) {
        newAnswers[i].points = 2;
      }
    }
  ...
  const validateTestData = (): boolean => {
    ...
    for (let i = 1; i <= questionCount; i++) {
      if (answers[i]) {
        totalPoints += answers[i].points || 2;
      }
    }
