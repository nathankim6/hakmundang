...
import { prepAnalysisCategories, prepSectionNames, allPrepQuestions, calculatePrepTotalMaxScore, getPrepQuestionPoints } from '@/data/prepLevelTestQuestions';
...
import BrainiacBulkAssignmentControls from './BrainiacBulkAssignmentControls';
...
const PrepLevelTestResultGroup = ({
  ...
  // 답안 기반으로 정답 여부 확인
  const isAnswerCorrect = (questionId: number, answer: any): boolean => {
    const question = allPrepQuestions.find(q => q.id === questionId);
  ...
  const calculateSubCategoryScores = (answers: Record<string, any>): Record<string, SubCategoryScore[]> => {
    ...
    Object.entries(prepAnalysisCategories).forEach(([sectionKey, categoryData]) => {
  ...
  const calculateSectionScores = (answers: Record<string, any>): SectionScore[] => {
    ...
    return Object.entries(prepAnalysisCategories).map(([sectionKey, categoryData]) => {
  ...
  const computeTotalScore = (answers: Record<string, any>): number => {
    let total = 0;
    allPrepQuestions.forEach(q => {
      if (isAnswerCorrect(q.id, answers[q.id])) {
  ...
  const handleDownloadImage = async (result: PrepLevelTestResult) => {
    ...
    try {
      ...
      link.href = url;
      const fileName = `${result.student_school || ''}${result.student_grade || ''}_${result.student_name}_초등부_BEAT_결과.jpg`;
  ...
  const PREP_TOTAL_POINTS = calculatePrepTotalMaxScore(); // Sum of all prep question points
        ...
        <div className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          {isExpanded && (
            <div className="flex items-center gap-2">
              <BrainiacBulkAssignmentControls results={results as any} />
              <Button 
                variant="outline" 
                size="sm" 
                      ...
                      <div className="relative z-10 p-3">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <div className="p-1.5 bg-white/10 rounded-lg border border-white/20">
                              <img 
                                ...
                                alt="Logo" 
                                className={`h-8 w-8 rounded ${result.academy === 'brainiac' ? 'object-contain bg-white' : 'object-cover'}`} 
                              />
                            </div>
                            <div>
                              <h1 className="font-bold text-base text-white tracking-tight">{result.academy === 'brainiac' ? '브래니악 영어 진단평가(BEAT) · 리포트' : '브래니악 영어 진단평가(BEAT) · 리포트'}</h1>
                              <p className="text-slate-300 text-[10px]">{result.academy === 'brainiac' ? 'Brainiac English Assessment Test Report' : 'Brainiac English Assessment Test Report'}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-white/10 border border-white/20">
                            ...
                            <span className="text-[10px] font-semibold text-white">BEAT · Prep</span>
                          </div>
                        </div>

                        {/* Student Info Grid */}
                        <div className={`grid ${result.academy === 'brainiac' ? 'grid-cols-3' : 'grid-cols-4'} gap-1.5`}>
                    ...
                    <div className="p-4 space-y-4">
                      ...
                      <div className="grid grid-cols-12 gap-3">
                        ...
                        {(() => {
                          ...
                          const top5Count = Math.max(1, Math.ceil(allScores.length * 0.05));
                          const top5Average = allScores.length > 0 
                            ? Math.round(allScores.slice(0, top5Count).reduce((acc, s) => acc + s, 0) / top5Count) 
                            : 0;
                          const top20Count = Math.max(1, Math.ceil(allScores.length * 0.2));
                          const top20Average = allScores.length > 0 
                            ? Math.round(allScores.slice(0, top20Count).reduce((acc, s) => acc + s, 0) / top20Count) 
                            : 0;
                          const isPartnerAcademy = !!result.academy && result.academy === 'brainiac';
                          const overallAverage = isPartnerAcademy
                            ? (allScores.length > 0 ? Math.round(allScores.reduce((acc, s) => acc + s, 0) / allScores.length) : 0)
                            : 171;
                              ...
                              {scoreCards.map(card => (
                                <div
                                  ...
                                  className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:-translate-y-0.5 ${
                                    card.highlight
                                      ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-slate-700/80 shadow-lg shadow-slate-900/30 ring-1 ring-amber-400/20'
                                      : 'bg-gradient-to-br from-white to-slate-50 border-slate-200/80 shadow-sm hover:shadow-md hover:border-slate-300'
                                  }`}
                                >
                                  {card.highlight && (
                                    <>
                                  ...
                                  <div className="relative p-2.5 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shadow-sm ${
                                        card.highlight
                                          ? 'bg-gradient-to-br from-amber-400/90 to-amber-600/90 shadow-amber-500/20'
                                          : 'bg-gradient-to-br from-slate-100 to-slate-200'
                                      }`}>
                                        <card.icon className={`w-4 h-4 ${
                                          card.highlight ? 'text-slate-900' : 'text-slate-600'
                                        }`} />
                                      </div>
                                      <p className={`text-[11px] font-semibold uppercase tracking-wider ${
                                        card.highlight ? 'text-amber-200/90' : 'text-slate-500'
                                      }`}>
                                        {card.label}
                                      </p>
                                    </div>
                                    <div className="flex items-baseline gap-0.5">
                                      <span className={`font-black tracking-tight tabular-nums ${
                                        card.highlight
                                          ? 'text-xl bg-gradient-to-b from-white to-slate-300 bg-clip-text text-transparent drop-shadow'
                                          : 'text-lg text-slate-800'
                                      }`}>
                                        {card.value}
                                      </span>
                                      {card.maxScore
                                        ? <span className={`text-[10px] font-medium ${card.highlight ? 'text-slate-400' : 'text-slate-400'}`}>/{card.maxScore}점</span>
                                        : <span className={`text-[10px] font-medium ${card.highlight ? 'text-slate-400' : 'text-slate-400'}`}>점</span>
                                      }
                                    </div>
                                  </div>
                                </div>
                        ...
                        {/* Radar Chart */}
                        <div className="col-span-8 bg-white rounded-lg p-2 border border-slate-100">
                          <p className="text-[10px] font-semibold text-slate-700 mb-1 text-center">영역별 분석</p>
                      ...
                      {result.academy === 'brainiac' && (
                        <div className="flex items-center justify-between gap-2 bg-gradient-to-r from-slate-50 via-white to-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                          <p className="text-[10px] font-semibold text-slate-600 tracking-wide uppercase">성취도 범례</p>
                          <div className="flex items-center gap-2">
                      ...
                      <div className={`rounded-xl p-4 shadow-sm border ${
                        result.academy === 'brainiac'
                          ? 'bg-gradient-to-br from-blue-50/80 via-white to-emerald-50/80 border-blue-200/80 shadow-blue-900/5'
                          : ''
                      }`}>
                        {result.academy === 'brainiac' && (
                          <p className="text-xs font-bold text-slate-700 mb-3 text-center tracking-wide">4대 영역 종합 성취도</p>
                        )}
                                  ...
                                  <>
                                    <div className="flex items-center justify-between mb-1">
                                      ...
                                      <button
                                        ...
                                        onClick={(e) => { e.stopPropagation(); cycleGrade(result.id, sectionKey, grade); }}
                                        title="클릭: A→B→C"
                                        className={`text-[9px] font-bold px-1.5 py-0.5 rounded cursor-pointer hover:ring-2 hover:ring-offset-1 hover:ring-slate-300 transition ${getGradeStyle(grade)}`}
                                      >
                                        {grade}
                                      </button>
                                    </div>
                      ...
                      <div className="space-y-3">
                        {Object.entries(prepAnalysisCategories).map(([sectionKey, categoryData]) => {
                          ...
                          return (
                            <div key={sectionKey} className="bg-white rounded-lg border border-slate-100 p-3">
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <div className={`p-1.5 rounded-md ${color.light}`}>
                                    {getSectionIcon(sectionKey)}
                                  </div>
                                  <h3 className="text-sm font-bold text-slate-800">{categoryData.name}</h3>
                                </div>
                                {result.academy !== 'brainiac' && (
                                      ...
                                      <>
                                        <div className="flex items-center justify-between mb-1">
                                          ...
                                          <button
                                            ...
                                            onClick={(e) => { e.stopPropagation(); cycleGrade(result.id, subKey, g); }}
                                            title="클릭: A→B→C"
                                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded cursor-pointer hover:ring-2 hover:ring-offset-1 hover:ring-slate-300 transition ${getGradeStyle(g)}`}
                                          >
                                            {g}
                                          </button>
                                          <span className={`text-[11px] font-semibold ${
                                            sub.percentage >= 70 ? 'text-emerald-600' : sub.percentage >= 40 ? 'text-amber-600' : 'text-red-600'
                                          }`}>
                      ...
                      <div className="text-center text-[9px] text-slate-400 pt-2 border-t border-slate-100">
                        © {result.academy === 'brainiac' ? 'BRAINIAC ENGLISH' : 'BRAINIAC ENGLISH'}. All rights reserved.
                      </div>
                    </div>
                  </div>
                )}
